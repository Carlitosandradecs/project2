...
#define CODE_LEN 75
#define NUM_OCC 505
// Structure to hold values from the Earnings-Dist-All-YYY.csv file.  See section
//1.1 of the
// project description on Data Format.
//
// A max-heap is defined as an array of struct SOC of size large enough to hold the
//occupations
...
struct SOC{
    ...
    int total; // Number of workers in the occupation that are year-round full-
    //time (YRFT)
    int female; // Number of YRFT that are female
    int male; // Number of YRFT that are male
};
// Structure to hold values from the Earnings-1960-1999.csv file.  See section 1.1
//of the project
...
struct earnings{
    ...
    int female_with_earnings; // Number of female workers (thousands) with
    //earnings
...
// for collision resolution.  Each entry contains a SOC code, and a pointer to the
//entry in
